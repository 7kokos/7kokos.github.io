...
$(function() {
	...
	$('button').on('click', (function(e){
		...
		if (k===0) {
			for (var i = 1; i < 4; i++) {
				...
				.append($('<span>Please enter ' + ($('.form-group:nth-child(' + i + ')').find('.form-control').attr('title')) + '</span>').addClass('tooltips'));
				};
	...
	$('.col-sm-10').hover(
  		...
  		$('span').remove();
  		k=0;
  	   	$(this).append($('<span>Please enter ' + ($(this).find('.form-control').attr('title')) + '</span>').addClass('tooltips'));

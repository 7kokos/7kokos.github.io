...
$(function () {
	...
	var info = [{
		...
		answer: ["Ответ 2", "Ответ 4", "Не знаю"],
		rightAnswer: {
			2: true
		}
	}, {
		question: "Вопрос #2 Сколько будет 1 + 3",
		answer: ["Ответ 4", "Ответ 3.14", "Это невозможно сложить"],
		rightAnswer: {
			1: true
		}
	}, {
		question: "Вопрос #3 Выберите четные числа (несколько вариантов ответа)",
		answer: ["Число 8", "Число 3", "Число 5", "Число 4"],
		rightAnswer: {
			1: true,
			4: true
		}
					...
					if (checked == true) {
						if (user[i][k] == true) {
							$(inputsShowResult[k]).attr({
								"disabled": true,
								"checked": true
							}).parent().append("<span> Правильный ответ!</span>").find("span").css({
								"color": "green"
							});
						} else {
							$(inputsShowResult[k]).attr({
								"disabled": true,
								"checked": true
							}).parent().append("<span> Неправильный ответ!</span>").find("span").css({
								"color": "red"
							});
						};
					} else {
